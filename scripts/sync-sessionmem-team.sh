...
ssh "$SURFACE_SSH" "powershell -NoProfile -Command \"\$env:Path = 'C:\\Users\\alfao\\.local\\bin;' + \$env:Path; cd 'C:\\Users\\alfao\\Documents\\GitHub\\Weftspun3DStudio'; if (Get-Command sessionmem -ErrorAction SilentlyContinue) { sessionmem sync } else { Write-Host 'sessionmem not on Surface PATH — skipped' }\""
