...
1. **Safety net**, tests covering the affected behavior are green right now. Uncovered? Write
   characterization tests FIRST: assert what the code currently does, even where that is ugly.
2. **Plan**, an ordered list of mechanical moves (rename / extract / inline / move / dedupe),
   each leaving the repo green. Post the list, that is the approval surface if the user is present.
