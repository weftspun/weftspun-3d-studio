...
2. Read: STATE watch-outs · PROJECT.md · scan DECISIONS.md + ISSUES.md Open (known issues in the
   ...
   files you will touch. List those files with one-line roles (feeds the design's Current state).
3. GATE: request conflicts with a Decision or a PROJECT constraint? Surface it to the user now, never design around it silently.
...
Per slice, in order:
1. Re-read that slice and the design sections it touches. Implement it completely, no TODO, no stub, no "for now".
...
(Better still: suggest the user run `workflows/review.md` in a fresh session that did not write this code.)
