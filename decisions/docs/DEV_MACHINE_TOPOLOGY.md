...
| Explicitly coding on DGX Remote | **DGX** | Lock → edit → `sync-changes-to-pc.sh --include-src --retry-until-complete` → give. Surface does **not** push until done. |
...
DGX is mid-edit on `src/`. **Stop.** Wait for DGX to finish and give, or confirm with the team which machine is canonical before using `-Force`.
