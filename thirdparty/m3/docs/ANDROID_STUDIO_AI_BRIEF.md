...
- Use the **main Weftspun3DStudio URL** (`https://<PC_IP>:3000/`), **not** `/xr` (IWSDK lab has no VRM / face relay UI).
...
| File | Role |
|------|------|
| `MainActivity.kt` | WebView, permissions, ⋮ → Chrome handoff, PiP, FaceKeeper get/release |
...
3. **Runtime permission**, make sure `FACE_TRACKING` granted before OpenXR `xrCreateFaceTrackerANDROID`.
