...
Wrong facts written here poison every future session, verify, do not guess. Anything you cannot
...
1. Get the brief from the user (or their message): what it is, stack, key constraints, how they
   want to run/test it. Ask only for what is missing and truly undecidable.
...
5. STATE / journal / DECISIONS initialized as in A.6 (record notable pre-existing choices you
   could infer, marked as observed, not decided). Make sure the repo's .gitignore covers
