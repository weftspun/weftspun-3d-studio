# Workflow: Review
For: reviewing a diff / PR / branch. Best run with fresh context, a session that did not write the
...
4. **Report**, per finding: `path:line`, what breaks, a concrete failing scenario, suggested
   ...
   Findings the user defers (or cannot act on now) → one ISSUES.md line each, so they survive.
